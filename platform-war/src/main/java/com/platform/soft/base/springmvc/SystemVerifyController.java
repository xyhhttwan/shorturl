...
public class SystemVerifyController {
    ...
    public void writeCode(HttpServletRequest request, HttpServletResponse response, int type) throws ServletException, IOException{
        //设置浏览器不缓存本页
        ...
        if(type==1){
            request.getSession().setAttribute(VerifyCode.VERIFY_TYPE_COMMENT_FRONT, verifyCode);

        }else if(type==2){
            request.getSession().setAttribute(VerifyCode.VERIFY_TYPE_COMMENT_FRONT_INQUIRY, verifyCode);
