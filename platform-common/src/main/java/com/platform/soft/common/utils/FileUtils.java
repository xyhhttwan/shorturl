...
public class FileUtils {
    ...
    public static boolean copyFileCover(String srcFileName,
        ...
        // 判断源文件是否存在
        if (!srcFile.exists()) {
            logger.error("复制文件失败，源文件{} 不存在!", srcFileName);
            return false;
        }
        // 判断源文件是否是合法的文件
        ...
        if (descFile.exists()) {
            ...
            if (coverlay) {
                ...
                if (!FileUtils.delFile(descFileName)) {
                    logger.error("删除目标文件{} 失败!", descFileName);
                    return false;
                }
            } else {
    ...
    public static boolean copyDirectoryCover(String srcDirName,
        ...
        // 判断源目录是否存在
        if (!srcDir.exists()) {
            logger.error("复制目录失败，源目录 {} 不存在!", srcDirName);
            return false;
        }
        // 判断源目录是否是目录
        else if (!srcDir.isDirectory()) {
            logger.error("复制目录失败，{} 不是一个目录!", srcDirName);
            return false;
        }
        ...
        if (descDir.exists()) {
            if (coverlay) {
                ...
                if (!FileUtils.delFile(descDirNames)) {
                    logger.error("删除目录 {} 失败!", descDirNames);
                    return false;
                }
            } else {
                logger.error("目标目录复制失败，目标目录 {} 已存在!", descDirNames);
                return false;
            }
        } else {
            // 创建目标目录
        ...
        if (!flag) {
            logger.error("复制目录 {} 到 {} 失败!", srcDirName, descDirName);
            return false;
        }
    ...
    public static void readFileByBytes(String fileName) {
        ...
        InputStream in = null;
        try {
           logger.debug("以字节为单位读取文件内容，一次读一个字节：");
            // 一次读一个字节
